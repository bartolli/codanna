...
impl SemanticVectorStorage {
    ...
    pub fn new(path: &Path, dimension: VectorDimension) -> Result<Self, SemanticSearchError> {
        ...
        let storage = MmapVectorStorage::new(path, SegmentOrdinal::new(0), dimension).map_err(|e| {
            SemanticSearchError::StorageError {
                message: format!("Failed to create storage: {e}"),
                suggestion: "Ensure the directory exists and you have write permissions".to_string(),
            }
        })?;
    ...
    pub fn save_embedding(
        ...
        let vector_id = VectorId::new(id.to_u32()).ok_or_else(|| SemanticSearchError::InvalidId {
            id: id.to_u32(),
    ...
    ///
    /// Legacy generations predate the finite-value ingress invariant. Validate
    /// every decoded vector here so NaN/±infinity on disk is treated as
    /// corruption rather than entering ranking state.
    pub fn load_all(&mut self) -> Result<Vec<(SymbolId, Vec<f32>)>, SemanticSearchError> {
        let vectors = self.storage.read_all_vectors().map_err(|e| {
            SemanticSearchError::StorageError {
                message: format!("Failed to load embeddings: {e}"),
                suggestion: "The storage file may be corrupted. Try rebuilding the semantic index."
                    .to_string(),
            }
        })?;
        ...
        for (vector_id, embedding) in vectors {
            self.dimension.validate_vector(&embedding).map_err(|e| {
                SemanticSearchError::StorageError {
                    ...
                    suggestion: "The semantic index is corrupt or was produced by an older unsafe build; rebuild it"
                        .to_string(),
                }
            })?;
    ...
    pub fn file_size(&self) -> Result<u64, SemanticSearchError> {
        self.storage.file_size().map_err(|e| SemanticSearchError::StorageError {
...
fn validate_physical_layout(storage: &MmapVectorStorage) -> Result<(), SemanticSearchError> {
    ...
    if actual != expected {
        return Err(SemanticSearchError::StorageError {
            ...
            suggestion: "The semantic index is incomplete or corrupt; rebuild it before loading".to_string(),
        });
    }
...
mod tests {
    ...
    fn test_save_and_load_single_embedding() {
        ...
        assert!(storage.load_embedding(SymbolId::new(999).unwrap()).is_none());
    }
    ...
    fn test_dimension_validation() {
        ...
        let dimension = VectorDimension::new(3).unwrap();
        let mut storage = SemanticVectorStorage::new(temp_dir.path(), dimension).unwrap();
        let result = storage.save_embedding(SymbolId::new(1).unwrap(), &[1.0, 2.0]);
    ...
    fn test_batch_operations() {
        ...
        assert_eq!(storage.load_embedding(SymbolId::new(10).unwrap()), Some(vec![1.0, 2.0]));
        assert_eq!(storage.load_embedding(SymbolId::new(20).unwrap()), Some(vec![3.0, 4.0]));
        assert_eq!(storage.load_embedding(SymbolId::new(30).unwrap()), Some(vec![5.0, 6.0]));
    }
    ...
    fn test_persistence_across_instances() {
        ...
        {
            let mut storage = SemanticVectorStorage::new(temp_dir.path(), dimension).unwrap();
            storage.save_embedding(SymbolId::new(42).unwrap(), &[1.5, 2.5]).unwrap();
        }
        {
    ...
    fn hardening_semantic_storage_rejects_impossible_header_count() {
        ...
        let dimension = VectorDimension::new(2).unwrap();
        let mut storage = SemanticVectorStorage::open_or_create(temp_dir.path(), dimension).unwrap();
        storage.save_embedding(SymbolId::new(1).unwrap(), &[1.0, 2.0]).unwrap();
        drop(storage);
